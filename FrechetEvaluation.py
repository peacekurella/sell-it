...
class FrechetEvaluation():
    ...
    def frechet_distance(self, samples_A, samples_B):
        ...
        _, generated_feat, _, _ = self.frechet_net(samples_A)
        _, real_feat, _, _ = self.frechet_net(samples_B)

        generated_feat = generated_feat.cpu().numpy()
        real_feat = real_feat.cpu().numpy()

        A_mu = np.mean(generated_feat, axis=0)
        A_sigma = np.cov(generated_feat, rowvar=False)
        B_mu = np.mean(real_feat, axis=0)
        B_sigma = np.cov(real_feat, rowvar=False)
